...
class WoocommerceArgentinaApi
  WOOCOMMERCE_ENDPOINT = ENV.fetch('WOOCOMMERCE_ARGENTINA_ENDPOINT')
  CONSUMER_KEY = ENV.fetch('ARGENTINA_WOOCOMMERCE_CONSUMER_KEY')
  CONSUMER_SECRET = ENV.fetch('ARGENTINA_WOOCOMMERCE_CONSUMER_SECRET')
  ...
  class << self
    ...
    def update_product(id, product)
      ...
      HTTParty.put(url, body: product.to_json, headers: { 'Content-Type': 'application/json' }, timeout: 300)
    end
