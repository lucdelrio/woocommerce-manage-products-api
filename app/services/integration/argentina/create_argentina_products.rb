...
class CreateArgentinaProducts
  class << self
    def iterate_categories_and_create_products
      ...
      categories.each_slice(2) do |group|
        group.each do |category|
          create_products_by_category(category.dig('description'))
        end
      end
    end
    ...
    def create_products_by_category(description)
      ...
      category.dig('id')
      products_list = ZecatArgentinaApi::Products.get_generic_product_by_family_with_pages(category.dig('id'), '1', '10')

      create_products_from_list(products_list['generic_products'])

      return if products_list.dig('total_pages') == 1

      (products_list.dig('total_pages')-1).times do |page|
        products_list = ZecatArgentinaApi::Products.get_generic_product_by_family_with_pages(category.dig('id'), page+2, '10')
        create_products_from_list(products_list['generic_products'])
      end
    end
    ...
    def create_product_images(full_product_list)
      full_product_list.each do |full_product|
        full_product[:product].dig('images').each_slice(4) do |group|
          WoocommerceApi.update_product(full_product[:id], {images: ZecatArgentinaApi::Products.fill_images_list(group)})
        end
      end
    end
  end
