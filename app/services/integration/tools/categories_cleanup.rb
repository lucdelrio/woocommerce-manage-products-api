      ...
      def iterate_categories_and_destroy
        Category.where(country: @zecat_country).each_slice(10) do |group|
          group.each do |local_category|
            zecat_category = CountrySelection.zecat_class_name(@zecat_country)::Families.category_by_id(
              @zecat_categories, local_category.zecat_id.to_s
            )
            ...
            local_category.destroy! if (zecat_category&.dig('show') == false) && !zecat_category.nil?
